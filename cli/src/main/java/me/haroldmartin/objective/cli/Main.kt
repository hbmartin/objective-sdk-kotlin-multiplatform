...
import me.haroldmartin.objective.cli.palettes.parseArgsForColorsPalette

fun main(args: Array<String>) {
    System.setProperty("slf4j.internal.verbosity", "ERROR")
    when {
        args.isEmpty() -> System.getenv("OBJECTIVE_KEY")?.let {
            runUi(arrayOf("-k", it))
        } ?: println(HELP_MESSAGE)
        args[0] == "-h" || args[0] == "--help" -> println(HELP_MESSAGE)
        args[0].startsWith("sk_") -> runUi(arrayOf("-k") + args)
        args[0] == "-k" -> runUi(args)
        else -> System.getenv("OBJECTIVE_KEY")?.let {
